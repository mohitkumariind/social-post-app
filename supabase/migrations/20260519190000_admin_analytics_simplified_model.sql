-- Simplified analytics: global DISTINCT user_id KPIs + fixed 7-day event metrics table.

CREATE OR REPLACE FUNCTION public.admin_engaged_users_count_scoped(
  p_from timestamptz,
  p_to timestamptz,
  p_scope_mode text,
...
BEGIN
  RETURN jsonb_build_object(
    'today',
      public.admin_engaged_users_count_scoped(
        v_start_today, v_now,
...
REVOKE ALL ON FUNCTION public.admin_engaged_users_count_scoped(
  timestamptz, timestamptz, text, bigint[], uuid, bigint[], text[]
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_engaged_users_count_scoped(
  timestamptz, timestamptz, text, bigint[], uuid, bigint[], text[]
) TO service_role;

REVOKE ALL ON FUNCTION public.admin_engaged_users_kpis(
  text, bigint[], uuid, bigint[], text[]
) FROM PUBLIC;
