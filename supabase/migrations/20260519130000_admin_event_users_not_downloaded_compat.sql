-- Fix PostgREST "function not found" when schema cache or clients still call the pre-engagement
-- 9-argument signature. Keep the 13-arg implementation; add a thin overload that delegates.

-- Ensure primary (engagement) implementation exists (idempotent).
...
-- Legacy 9-arg signature (pre-engagement RPC) — delegates with open-ended date filters.
CREATE OR REPLACE FUNCTION public.admin_event_users_not_downloaded_page(
  p_event_id uuid,
  p_scope_mode text,
  p_moderator_state_ids bigint[],
...
SET row_security = off
AS $$
  SELECT public.admin_event_users_not_downloaded_page(
    p_event_id,
    p_scope_mode,
