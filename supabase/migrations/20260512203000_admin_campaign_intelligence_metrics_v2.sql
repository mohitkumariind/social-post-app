-- Campaign Intelligence v2: metric sources + GROUP BY event_id (NULL = "Global").
--
-- Sent      → SUM(notification_broadcasts.target_user_count) GROUP BY nb.event_id
-- Delivered → COUNT(notifications_history) WHERE delivery_status = 'sent' GROUP BY nb.event_id
-- Opened    → COUNT(notification_open) GROUP BY nb.event_id
-- Downloads → COUNT(post_downloads) via posts.event_id GROUP BY p.event_id
--
-- Rows: scoped events (search + RBAC) plus optional synthetic "Global" row (event_id NULL) when
-- any global-scoped metric exists and p_event_id drill filter is not set.

-- Ensure worker columns exist (idempotent; mirrors 20260509195500 for DBs that skipped that migration).
