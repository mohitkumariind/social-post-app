...
export const POST = withAudit(
  async ({ req, auth, admin, previous_data }) => {
    ...
    const { data: rpcRes, error: rpcErr } = await admin.rpc('twitter_campaign_pause', { p_campaign_id: id });
    if (rpcErr) return json({ error: rpcErr.message }, 500);
    const body = (rpcRes ?? {}) as { ok?: boolean; error?: string; status?: string };
    if (!body.ok) return json(body, 409);
  ...
  {
    action_type: 'twitter_campaigns.pause',
